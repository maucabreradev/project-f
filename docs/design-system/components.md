...
| Level | Project F owns | Mapped from shadcn-solid / Kobalte |
| --- | --- | --- |
| Atoms | `Avatar`, `Currency`, `RankTag`, `Icon` (via icon library), `UserLink`, `PostMeta` | `Button`, `Input`, `Textarea`, `Select`, `Label`, `Badge`, `Separator`, `Switch`, `Checkbox`, `Spinner`, `Skeleton` |
| Molecules | `SearchField`, `BalancePill`, `EmptyState`, `ToastFeedback` (wraps sonner), `InlineError`, `ThreadRow`, `BoardRow`, `CategorySection`, `PostProfile`, `PaginationControls` | `Card`, `Dialog`, `AlertDialog`, `Sheet`, `Popover`, `DropdownMenu`, `Tooltip`, `Tabs`, `Table`, `Breadcrumb`, `Field`, `Form` (Zod), `Alert`, `Sonner`, `Pagination` |
| Organisms | `FloatingNavbar`, `ForumHero` (banner), `BoardSidebar/BoardStructure`, `PostList`, `SkeletonPage`, `ErrorPage`, `BanInterstitial` | `Sheet` (mobile drawer), `Tabs` (admin) |
| Templates | Page layouts (home, board, thread, admin, auth) | `layout.md` |
...
- **SkeletonPage** (organism) — layout-preserving skeleton for boards/threads/admin tabs (header + rows), never a full-page freeze.
...
- **ErrorPage** (organism) — full-page states:
