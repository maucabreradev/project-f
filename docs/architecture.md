...
│   │   ├── src/components/     # Atomic Design: atoms / molecules / organisms / templates (catalog: docs/design-system/component-architecture.md)
│   │   ├── src/layouts/
...
- **Styling**: Tailwind CSS v4 tokens with Kobalte primitives and shadcn-solid components; Atomic Design organization (AGENTS.md rule 20). The approved Design System lives in `docs/design-system/` (tokens, components, component-architecture, layout, accessibility) — frontend work references it as the single source of truth.
