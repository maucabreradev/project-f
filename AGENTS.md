...
34. When an issue-tracker ticket is finished, set its `Status:` line to `done` and append a `## Completion` note (commit and date). `done` requires the ticket's acceptance criteria and the quality gate (rule 28). Continue by working the frontier ticket — the earliest one whose blockers are all `done`.

## Repository state

There is no application code, package manifest, build/test system, or CI yet — the tracked content is `LICENSE`, the documentation (`CONTEXT.md`, `README.md`, `docs/domain-model.md`, `docs/adr/0001-0006`, `docs/architecture.md`, `docs/persistence-model.md`, `docs/api.md`, the approved Design System `docs/design-system/`, the accepted UX flows `docs/ux/`, the accepted route map `docs/routes.md`, the engineering-skill config `docs/agents/`), the product specs and implementation tickets (`.scratch/initial-product-definition/`, and the consolidated spec with its vertical-slice tickets 01–34 in `.scratch/project-f-spec/`), a proposed feature spec (`.scratch/member-directory/`), and the agent skills (`.agents/`, pinned by `skills-lock.json`). The domain documentation is complete and authoritative: read `CONTEXT.md` (glossary), `docs/domain-model.md` (entities, rules, invariants), and `docs/adr/` (decisions) before any design or implementation work; then follow the approved architecture (`docs/architecture.md`), the accepted persistence model (`docs/persistence-model.md`), the accepted REST API design (`docs/api.md`), the approved Design System (`docs/design-system/`), the approved UX flows (`docs/ux/`), and the route map (`docs/routes.md`). `README.md` lays out the document hierarchy in one place. Don't hunt for entrypoints or conventions in code; there is none yet.
